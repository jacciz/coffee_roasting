...
server <- function(input, output, session) {
    ...
    #  -------------- The Roast Profile Analysis Table --------------------  
    ...
    output$coffee_tasting <- renderPlotly2({
        ...
        as_widget(p) %>% onRender(addClickBehavior)#onRender(addHoverBehavior)
    })
...
# Make dropdown menu of all countries, roast machines, 
    output$get_country <- renderUI({
        ...
        selectInput("country", "Country", countries, width = '100%', selected = NULL)
    })
    output$get_roast_machine <- renderUI({
        machines <- coffee_producting_countries %>% sort()
        selectInput("roast_machine", "Machine", coffee_roasting_machines, width = '100%', selected = NULL)
    })
    output$get_processing_method <- renderUI({
        method <- processing_methods %>% sort()
        selectInput("processing_method", "Processing method", method, width = '100%', selected = NULL)
    })
    output$get_variety <- renderUI({
        varieties <- coffee_varieties_arabica %>% sort()
        selectInput("variety", "Variety", varieties, width = '100%', selected = NULL)
    })
    #  -------------- Input Roast Data from file/user --------------------  
    ...
    observeEvent(input$update_record, {
        ...
        # First, save the xlsx that was uploads into the server as a csv
        if (!is.null(input$roast_curves_upload)){
            df <- readxl::read_xlsx(input$roast_curves_upload$datapath, skip = 3) %>% select(1:6) %>% rename(change_BT = "Δ BT")
            # Create filename based on country and a timestamp
            filename = paste0("roast_profiles_xlsx/",input$country,"-", paste0(format(Sys.time(), "%d-%b-%Y-%H-%M"), ".csv"))
            write.csv(df, filename)
        }
        save_filename <- c("filename" = filename) %>% as.list()
        # as.list("country" = "as")
        # file.copy(input$roast_curves_upload$datapath, "/roast_profiles_xlsx")  }
        print(class(form_data_contact_db()))
        record_status <- save_data_in_roast_profiles(form_data_contact_db(), "roast_profiles") # name of db table
        save_data_in_roast_profiles(save_filename, "roast_profiles") # name of db table
        list_data <- loadData(c("primary_key", "roast_date", "country"), "roast_profiles") %>% as.data.frame()

        # if(record_status == "inserted."){ # Dont need this??
    ...
    observeEvent(input$new_record, {
        ...
        updateTextInput(session, inputId = "roast_date", value = as.Date(Sys.Date()))
        ...
        # updateTextInput(inputId = "Primary_Contact_Name", value = loaded_form_data$Primary_Contact_Name)
